...
import argparse
import torch
from models.ResUnet_cls import S12twin_shift, MLPHead
from datasets.datasets_flood_ori import OSCD
...
def parse_option():
    ...
    # 1600
    parser.add_argument('--batch_size', type=int, default=1000, help='batch_size') #30->256; 10->64
    ...
    # add new views
    parser.add_argument('--data_dir_train', type=str, default='/workplace/Floods64', help='path to training dataset')
    ...
    parser.add_argument('--model_path', type=str, default='./save_twins12_shift_crossvq', help='path to save model')
    parser.add_argument('--save', type=str, default='./save_twins12_shift_crossvq', help='path to save linear classifier')
...
def get_train_loader(args):
    # load datasets
    train_set = OSCD(args.data_dir_train,
                    subset="train",
...
class Trainer:
    ...
    def train(self, train_loader):
        ...
        for epoch_counter in range(self.max_epochs):
            ...
            for idx, batch in enumerate(train_loader):
                ...
                sl8 = batch['sl8']
                ss1 = batch['ss1']
                loss = self.update(image, sl8, ss1)
    ...
    def update(self, image, sl8, ss1):
        ...
        batch_sl8 = sl8[:, 8:24, 8:24]
        batch_ss1 = ss1[:, 8:24, 8:24]
        # compute query feature
        feature1, feature2 = self.online_network(batch_view_1, batch_view_2, mode=0)
        feature1 = self.predictor(feature1)
        feature2 = self.predictor(feature2)
        feature2 = self.aug_list(feature2, model, param)
        # compute key features
        with torch.no_grad():
            t_feature1, t_feature2 = self.target_network(batch_view_1, batch_view_2, mode=0)
            t_feature2 = self.aug_list(t_feature2, model, param)
            # generating_mask
            batch_sl8 = batch_sl8.unsqueeze(dim=1)
            batch_sl8 = self.aug_list(batch_sl8.float(), model, param)[:, 0, :, :]
            batch_ss1 = batch_ss1.unsqueeze(dim=1)
            batch_ss1 = self.aug_list(batch_ss1.float(), model, param)[:, 0, :, :]
            l8_list, s1_list = self.mask_spix(batch_sl8, batch_ss1)

        batch_sl8 = batch_sl8.long().to(self.device)
        batch_ss1 = batch_ss1.long().to(self.device)
        feature1, t_feature1 = self.avg_main_cls(batch_sl8, l8_list, feature1, t_feature1)
        feature2, t_feature2 = self.avg_main_cls(batch_ss1, s1_list, feature2, t_feature2)
...
def main():
    ...
    # set the model
    online_network = S12twin_shift(width=1, in_channel1=4, in_channel2=2, in_dim=args.in_dim, feat_dim=args.feat_dim).to(device)
    ...
    ## optimizer
    #--> target model
    optimizer = torch.optim.Adam(list(online_network.parameters()) + list(predictor.parameters()), lr=3e-4,
                                 weight_decay=1e-4)
