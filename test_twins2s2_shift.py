...
def parse_option():
    ...
    # output
    parser.add_argument('--out_dir', type=str, default='./result_twins2s2_shift2_InferS2_all_pix', help='path to save linear classifier')
    parser.add_argument('--score', action='store_true', default=True, help='score prediction results using ground-truth data')
    parser.add_argument('--preview_dir', type=str, default='./preview_twins2s2_shift2_InferS2_all_pix', help='path to preview dir (default: no previews)')
...
def encoder_factory(model, args):
    ...
    #pretrained_model = os.path.join('./save_twins2s2_shift_spix', 'twins_epoch_199_15.387975284031459.pth')
    pretrained_model = os.path.join('./save_twins2s2_shift_pixTS/resunet18_crop_32_fetdim_128', 'twins_epoch_199_11.142740249633789.pth')
    ...
    with torch.no_grad():
        for idx, (batch) in enumerate(val_loader):
            ...
            if args.score:
                ...
                image = batch['image']
            # crop and output
            cd_img = Crop_img(image, 128, 0.8, classifier, args) #128 for small  / 256 for big
