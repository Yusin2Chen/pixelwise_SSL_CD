...
def load_sample(sample, use_s1, use_s2hr, use_s2mr, use_s2lr,
    ...
    # load s2 data
    if use_s2:
        img = load_l8(sample["s2"], use_s2hr, use_s2mr, use_s2lr)
    ...
    #拼接成4倍大小
    #img_array = np.concatenate((img, img), axis=2)
...
class DFC2020(data.Dataset):
    ...
    def __init__(self,
        ...
        if subset == "train":
            ...
            train_list = [x for x in train_list if "l8_" in x]
            sample_dirs = train_list
            #path = os.path.join(path, "ROIs0000_validation", "s2_validation")
        else:
            path = os.path.join(path, "ROIs0000_test", "l8_0")
            sample_dirs = []
        ...
        for folder in sample_dirs:
            ...
            for s2_loc in tqdm(s2_locations, desc="[Load]"):
                s1_loc = s2_loc.replace("_al8_", "_bs1_").replace("al8_", "bs1_")
                lc_loc = s2_loc.replace("_al8_", "_dfc_").replace("al8_", "dfc_")
