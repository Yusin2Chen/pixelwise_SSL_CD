...
from models.VIReg_Unet import twinshift, MLPHead
from utils.loss_cls import sim_loss, cov_loss, std_loss
...
def parse_option():
    ...
    parser.add_argument('--model_path', type=str, default='./save_VIReg_shift_spixTS', help='path to save model')
    parser.add_argument('--save', type=str, default='./save_VIReg_shift_spixTS', help='path to save linear classifier')
...
class Trainer:
    ...
    def update(self, image, seb, ses):
        ...
        feature1, vec1 = self.online_network(batch_view_1, mode=0)
        feature2, vec2 = self.online_network(batch_view_2, mode=0)
        vec1 = vec1.squeeze()
        vec2 = vec2.squeeze()
        # alignment
        ...
        feature1 = self.avg_main_cls(batch_segm_b, big_list, feature1)
        feature2 = self.avg_main_cls(batch_segm_s, small_list, feature2)

        v_sim_loss = sim_loss(vec1, vec2)
        v_std_loss = std_loss(vec1, vec2)
        v_cov_loss = cov_loss(vec1, vec2)


        l_sim_loss = sim_loss(feature1, feature2)
        l_std_loss = std_loss(feature1, feature2)
        l_cov_loss = cov_loss(feature1, feature2)

        loss_v = self.ll * v_sim_loss + self.mu * v_std_loss + self.nu * v_cov_loss
        loss_l = self.ll * l_sim_loss + self.mu * l_std_loss + self.nu * l_cov_loss

        return loss_v + loss_l
    ...
    def avg_main_cls(self, batch_segm, batch_list, inFeats1):
